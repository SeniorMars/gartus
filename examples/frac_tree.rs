...
fn draw_fractal_tree(
    ...
    turtle
        .pop_state()
        .expect("turtle state stack should contain left branch state"); // Restore the previous turtle state
    ...
    turtle
        .pop_state()
        .expect("turtle state stack should contain right branch state"); // Restore the previous turtle state
}
...
fn main() {
    ...
    let mut turtle = Turtle::new(Rgb::new(0, 128, 0), 90.0, (width / 2) as f64, height as f64);
    turtle.pen_down();
    ...
    turtle.set_position((width / 2) as f64, height as f64);

//! Grid-baked procedural volume example.
//!
//! This bakes a curl-warped procedural smoke field into a `GridDensityField`, then renders the grid
//! through `NonUniformMedium`. The same grid can be saved with `save_raw` and loaded later.
//!
//! Outputs a PNG to `final/raytracing/grid_volume.png`.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let path = "final/raytracing/grid_volume.png";
...
fn render_frame(frame: usize) -> Canvas {
    ...
    let camera = RayCamera::new(IMAGE_WIDTH, 16.0 / 9.0)
        ...
        .with_look_at(Point::new(5.8, 2.8, 6.8), Point::new(0.0, 0.8, 0.0))
...
fn build_scene(time: f64) -> (HittableList, WeightedSamplingTargetList) {
    ...
    let light_corner = Point::new(-2.5, 4.6, -0.6);
    ...
    let grid_bounds = GridBounds::new(
        smoke_center - Vector::new(smoke_radius, smoke_radius, smoke_radius),
        smoke_center + Vector::new(smoke_radius, smoke_radius, smoke_radius),
    );
    let procedural_smoke = ProceduralDensityField::smoke()
        .with_seed(10)
        .with_scale(1.5)
        .with_speed(0.35)
        ...
        .with_warp_speed(0.5);
    let smoke_density =
        GridDensityField::from_density_field(grid_bounds, [48, 48, 48], &procedural_smoke, time)
